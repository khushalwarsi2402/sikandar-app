...
// ============================================
const API_BASE = '/api';
let authToken = localStorage.getItem('srm_token') || null;
...
async function handleLogin() {
  ...
  try {
    ...
    localStorage.setItem('srm_token', authToken);
    onAuthSuccess();
  } catch (err) {
...
async function handleRegister() {
  ...
  try {
    const data = await apiCall('/auth/register', 'POST', { name, email, phone, password });
    authToken = data.token;
    currentUser = data.user;
    localStorage.setItem('srm_token', authToken);
    onAuthSuccess();
...
function onAuthSuccess() {
  ...
  if (currentUser) {
    ...
    userProfile.dob = currentUser.dob || '';
    userProfile.avatar = currentUser.avatar || '👤';
    userProfile.addresses = currentUser.addresses || [];
    userProfile.payments = currentUser.savedPayments || [];
    if (currentUser.preferences) {
      userProfile.prefs = {
        notifications: currentUser.preferences.notifications,
        emails: currentUser.preferences.emails,
  ...
  showToast(`🎉 Welcome${currentUser ? ', ' + currentUser.name : ''}!`);
}
...
function updateAuthUI() {
  ...
  if (authToken && currentUser) {
    ...
    profileBtn.setAttribute('onclick', 'toggleProfile()');
    profileBtn.setAttribute('title', 'My Profile');

    // Update profile display name
    if (document.getElementById('profile-display-name')) {
      document.getElementById('profile-display-name').textContent = currentUser.name || 'User';
    }
    if (document.getElementById('profile-display-phone')) {
      document.getElementById('profile-display-phone').textContent = currentUser.phone || currentUser.email;
    }

    // Update logout button text
    const logoutBtn = document.querySelector('.profile-logout-btn');
    if (logoutBtn) logoutBtn.innerHTML = '🚪 Sign Out';
  } else {
    // Signed out — show sign in
    navAvatar.textContent = '👤';
    profileBtn.setAttribute('onclick', 'openAuthModal()');
...
function connectSocket() {
  ...
  try {
    ...
    socket.on('statusUpdate', (data) => {
      ...
      // You could update tracking UI here for live updates
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Load profile from localStorage first (works in both modes)
  ...
  // Try to authenticate with backend (if running with server)
  checkAuth();
});
